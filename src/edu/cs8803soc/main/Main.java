...
public class Main {
	...
	public static void main(String[] args) {
		...
		// id.importLists();
		Thread businessImportThread = new Thread(new DataOperation(id, DataOperation.BUSINESS));
		businessImportThread.start();
		
		Thread reviewImportThread = new Thread(new DataOperation(id, DataOperation.REVIEW));
		reviewImportThread.start();
		
		Thread tipImportThread = new Thread(new DataOperation(id, DataOperation.TIP));
		tipImportThread.start();
		
		Thread checkingImportThread = new Thread(new DataOperation(id, DataOperation.CHECKIN));
